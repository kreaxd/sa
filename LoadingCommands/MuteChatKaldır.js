module.exports.operate = async ({client, msg, args, author, member, auth}, Database = require("../Models/Restriction.js")) => {
    ...
    if (author.roles.highest.position <= member.roles.highest.position) return client.message(client.embed("Bu kişi senden yüksek veya aynı yetkiye sahip olduğu için mute cezasını kaldıramazsın.", msg), msg.channel.id, 5000);
    let reason = args.splice(1).join(" ") || "Sebep Girilmedi.";
    let Data = await Database.find({Activity: true, userID: member.id, Type: "MUTE" });
    if ((Data.length <= 0) & (!member.roles.cache.get(auth.CezaRoles.MuteRoles))) return client.message(client.embed("Belirttiğin kişi herhangi bir chat mute cezasına sahip değil!", msg), msg.channel.id, 5500);
