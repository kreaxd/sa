module.exports.operate = async ({client, msg, args, member, auth, author}, ms = require("ms"), Discord = require("discord.js"), Database = require("../Models/Restriction.js")) => {
    ...
    if (!args[1]) {
        Database.find({userID: member.id, Activity: true, $or: [{Type: "MUTE"}, {Type: "VOICEMUTE"}]}, async(err, res) => {
          if (res.length <= 0) return client.message(client.embed(`Kullanıcının ceza listesinde aktif bir aktif ceza bulunmuyor!`, msg), msg.channel.id, 5500);
          res = res.reverse();
        ...
        Database.find({userID: member.id, Activity: true, Type: "VOICEMUTE"}, async (err,res) => {
          if (res.length <= 0) return client.message(client.embed(`Kullanıcının ceza listesinde herhangi bir aktif ceza bulunmuyor!`, msg), msg.channel.id, 5500);
          res = res.reverse();
          let sesveri = res.map((x, index) => `\`${index + 1}.\` **[VoiceMute/CezaID: #${x.CezaID}]** var olan cezadan geriye \`${client.format((x.FinishDate - Date.now()))}\` kaldı.(Ceza Veren: <@${x.Author}>)`)
        ...
        Database.find({userID: member.id, Activity: true, Type: "MUTE"}, async (err,res) => {
          if (res.length <= 0) return client.message(client.embed(`Kullanıcının ceza listesinde herhangi bir aktif ceza bulunmuyor!`, msg), msg.channel.id, 5500);
          res = res.reverse();
          let sesveri = res.map((x, index) => `\`${index + 1}.\` **[ChatMute/CezaID: #${x.CezaID}]** var olan cezadan geriye \`${client.format((x.FinishDate - Date.now()))}\` kaldı.(Ceza Veren: <@${x.Author}>)`)
          ...
          client.message(client.embed(`Hatalı bir argüman girmeye çalışıyorsun. Ex: chat,voice/metin,ses`, msg), msg.channel.id, 7500);
    }
  };
