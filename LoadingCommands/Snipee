...
if(!member) return msg.channel.send({embed: {description: `Bir kullanıcı belirtmelisin!`}}).then(x => x.delete({ timeout: 5000 }));
    ...
    if(!kanal) return msg.channel.send({embed: {description: `Belirttiğin kullanıcı bir sesli kanalda b!`}}).then(x => x.delete({ timeout: 5000 }));
...
msg.channel.send({embed: {description: `${member} kullanıcısı \`${kanal.name}\` kanalında.
