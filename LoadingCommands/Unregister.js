module.exports.operate = async ({client, msg, args, member,auth, author}) => { 
  ...
  if (member.user.username.includes(auth.Tags.RealTag)) return client.message(client.embed("Kullanıcının isminde tagımız bulunuyor bu yüzden işlem iptal edildi.",msg),msg.channel.id,6500);
  if (author.roles.cache.some(r => auth.Perms.Unregister.includes(r.id))) return client.message(client.embed("Kullanıcı zaten kayıtsızda bulunuyor bu yüzden kayıtsız atamam.",msg),msg.channel.id,6500);
